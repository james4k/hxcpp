...
#define WINRT_LOG(fmt, ...) {char buf[1024];_snprintf(buf,1024,"****LOG: %s(%d): %s \n    [" fmt "]\n",__FILE__,__LINE__,__FUNCTION__, __VA_ARGS__);OutputDebugString(buf);}
#define WINRT_PRINTF(fmt, ...) {char buf[2048];_snprintf(buf,2048,fmt,__VA_ARGS__);OutputDebugString(buf);}
